...
- [x] **10.1** OOM prevention — verify no full-res image decoding: all thumbnail `Image` objects have `sourceSize` set (256×160); `source` gated on `root.visible`; `Repeater` in singletons only renders when `visible`; `Loader` per tab keeps heavy lists from building at startup
 ✅ *audited live: all 6 plain `Image` elements carry sourceSize (88–512 px) AND visibility-gated sources; picker is spine+stage now — exactly ONE preview decoded, size clamped to frame; settings/control tabs are Loaders (6+3); no Repeater builds unbounded delegates in singletons*
- [x] **10.2** Race condition fixes — `Wallpaper.apply()` not synchronously after `FileView.reload()` (relies on `onLoaded` handler); `DisplayService.setBright()` not called synchronously on hover; `Notify` not touched after `n.closed.connect()` — check `dead` before closing
 ✅ *wallpaper tokens apply only via onLoaded handlers (sync path deleted long ago, comments document it); setBright fires solely from OSD slider drag — no hover path exists; Notify guards every close with vm.dead and never touches objects after closed*
- [x] **10.3** QML error handling — `console.warn` during boot-phase diagnostics (not `console.log`); `Component.onCompleted` errors are edit-order artifacts against live instance, not code bugs; verify against freshly spawned instance
 ✅ *boot-phase diagnostics stream to the authoritative by-pid log.log (verified: preset applied / bins resolved / Configuration Loaded sequence); remaining console.log sites are runtime event traces, not boot probes*
- [x] **10.4** PID tracking — never `pkill -f "quickshell -p <path>"` (matches user's live instance); record test instance PID at spawn (`$!` from `setsid`) and kill exact PID only
 ✅ *every spawn this session captured $! from setsid and killed that exact PID; zero pattern-kills*
- [x] **10.5** Config persistence — `ShellState.set()` → `state.json`; `ShellState.flushNow()` for session-ending paths; state seeds only when file absent/empty (transient read race never clobbers user prefs)
 ✅ *flushNow wired into logout() and all user-paced plugin mutations; state seeds only when file absent/empty (ShellState.qml:442)*
- [x] **10.5** Linting & formatting — run `make lint-qml` after `qs -p .`; run `qmlfmt -t 4 -i 4 -b 250 -w **/*.qml`; run `gofmt -w .`; run `go mod tidy`; run `golangci-lint run`
 ✅ *make lint-qml re-run green on final tree (85 files, 0 syntax errors); Go tooling N/A (pure QML); qmlformat stays opt-in per PH.08 — running it would reflow all 85 files for zero behavioral gain*
- [x] **10.6** Background quickshell between bash calls — `setsid nohup ... < /dev/null & disown` pattern; plain nohup gets reaped when tool session ends
 ✅ *followed every spawn this session; smoke.sh encodes the pattern*
- [x] **10.7** Console logging — `qs -p <path> log > file &` to capture stderr; `console.log` from QML does NOT reliably reach nohup-captured stderr; use `warn` for boot-phase diagnostics
 ✅ *all verification read /run/user/1000/quickshell/by-pid/<pid>/log.log directly — the authoritative channel*
- [x] **10.8** IPC targeting — `qs ipc call ...` without `--pid`/`--id` targets exactly ONE instance (first found, usually live shell); to drive specific instance: `qs ipc --pid <pid> call <target> <fn>`; or `-i <id-substring>`; bare `-p` targets live session — note prefs from logs first and restore after, or target test PID directly
 ✅ *every mutating call this session used qs ipc --pid <test-pid>; user prefs recorded before and restored after each phase run*
- [x] **10.9** Failed-config instances — editing QML while instance runs triggers hot-reload mid-edit-sequence: saving caller before callee produces transient `ReferenceError`s; don't chase in log captures; verify against freshly spawned instance instead
 ✅ *settled code verified only against freshly spawned instances throughout*
- [x] **10.10** Final RSS check — `ps -o rss -p $(pgrep -f 'qs -c')` should be 250-500 MB; tail clean-load log: `tail -n 30 /run/user/1000/quickshell/by-pid/$(pgrep -f 'qs -c')/log.log`; `qs ipc -c yuta-qs call panel toggle` exercises panel via IPC
 ✅ *final battery: YUTA_DEBUG_CYCLE walk of all 15 settings pages (0 errors, 405 MB), make test SMOKE OK (0 errors, 418 MB), clean-load tail verified (Configuration Loaded → palette applied → bins resolved), panel toggle exercised, RSS range 367–418 MB across runs*
